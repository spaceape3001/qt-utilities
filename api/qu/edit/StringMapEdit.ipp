...
namespace qu {
    ...
    StringMap	StringMapEdit::get() const
    {
        return m_model->get();
    }

    void		StringMapEdit::set(const StringMap&v)
    {
        m_model->set(v);
    }
    ...
    StringMap		StringMapEdit::Model::get() const
    {
        StringMap	ret;
        for(auto& i : m_items){
            if(i.key.isEmpty())
    ...
    void			StringMapEdit::Model::set(const StringMap&k)
    {
        beginResetModel();
        m_items.clear();
        for(auto& i : k)
